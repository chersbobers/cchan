...
\`\`\`sql
-- Create images table (supports text-only posts)
CREATE TABLE images (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  title VARCHAR(200) NOT NULL,
  description TEXT,
  image_url VARCHAR(500), -- Now nullable to support text-only posts
...
For posts with images, use Supabase Storage:
...
Then save metadata to the \`images\` table. The \`image_url\` field can be null for text-only posts.
